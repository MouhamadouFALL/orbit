...
class PaymentDetails(models.Model):
    ...
    token_status = fields.Boolean(string='Token Status', required=False)
    ...
    payment_token = fields.Char(string='Payment Token', required=False)
    ...
    partner_id = fields.Integer(string='Partner ID', required=False)    
    ...
    payment_state = fields.Selection([
        ...
        ('completed', 'Completed'),
        ('failed', 'Failed')
    ], string='Payment State', required=False)
    
    account_payment_id = fields.Many2one('account.payment', string="Paiement généré", readonly=True)
    
    _sql_constraints = [
        ('unique_transaction_id', 'unique(transaction_id)', 'Transaction ID déjà enregistré.'),
    ]
